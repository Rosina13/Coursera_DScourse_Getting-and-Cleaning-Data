...
#create a second, independent tidy data set with the averages per activity and per subject
  library(dplyr)
  dataset4<-group_by_at(dataset3,.vars = c("subjects","activity")) 
  tidydata<-summarize_at(dataset4,.vars = names(dataset4)[3:68],.funs = "mean")
...
#write codebook
  setwd("F:/Coursera - Data Science/Course 3/week 4")
  codebook <- data.frame("variable_name"=1:68,"levels"=1:68) #create empty dataframe
  codebook$variable_name <- names(dataset3)
      
  #add the levels that variables can have
  codebook$levels[1] <- "1:30"
  codebook$levels[2] <- paste(levels(as.factor(dataset3$activity)),collapse=", ")
  codebook$levels[3:68] <- "not applicable"
  write.table(codebook,"codebook.txt",row.name=FALSE)
